...
ConvexPolygonNester::ConvexPolygonNester(
        const vector<Polygon>& inputPolygons,
        const Rectangled& bin): bin(bin),
...
void ConvexPolygonNester::setBin(const Rectangled& newBin) {
    bin = newBin;
    placer.setBin(newBin);
}
void ConvexPolygonNester::setInputPolygons(
        const vector<Polygon>& newInputPolygons) {
