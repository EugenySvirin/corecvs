...
void putInConteiner(T1& conteiner, const T2& obj, int amount) {
...
void ok() {
    ...
    Polygon A = {{0, 0} , {0, 10}, {10, 0}};
    Polygon B = {{0, 0} , {0, 10}, {10, 10}, {10, 0}};
    Polygon C = {{1.5, 1.5}, {0, 2.5}, {2.5, 4.5}, {4.5, 3.5}, {3.5, 1.5}};

    vector<Polygon> polygons = {A, A, A, A, A, A, A};
    ...
    auto toShow = placer.blPRMassPriority(polygons, 16);
    for (auto& pol : toShow) {
        //showPolygon(pol);
    }
    drawSvgPolygons(toShow, 250, 250, "placementTesting");
}

double getMaxValueY(const vector<corecvs::Polygon> &inputList) {
    double max = 0;
...
void runTest(string filename, vector<Polygon>& polygons,
     ...
     auto svgnestToPlace = polygons;
     for (auto i = 0 ; i < svgnestToPlace.size(); ++i) {
         svgnestToPlace[i].translate({i * 10, i * 10});
     }
     svgnestToPlace.push_back(polFromRec(bin));
     drawSvgPolygons(svgnestToPlace, 400 + bin.height(), 400 + bin.width(), filename + " for svg to run.svg");
...
int main(int argc, char **argv) {
    ...
    using namespace corecvs;
        ...
        //runTest("testPronest1", ProNest1, areaPN1); тест с деталью
        ...
        Rectangled beutifulArea(10, 10, 150, 150);
        vector<Polygon> inputList7;
        putInConteiner(inputList7, quad, 20);
        putInConteiner(inputList7, pentaR, 20);
        putInConteiner(inputList7, getHomotheticPolygon(geksaR, 0.25), 70);

        ConvexPolygonNester nester;
        nester.setInputPolygons(inputList7);
        nester.setBin(beutifulArea);
        nester.run_FFD_LM_BLPR(4);
        auto polsFFD = nester.getPlacedPolygons();
        nester.run_FFI_BLPR(4);
        auto polsFFI = nester.getPlacedPolygons();

        polsFFD.push_back(polFromRec(beutifulArea));
        polsFFI.push_back(polFromRec(beutifulArea));
        drawSvgPolygons(polsFFD, 170, 170, "FFDSample.svg");
        drawSvgPolygons(polsFFI, 170, 170, "FFISample.svg");
}
